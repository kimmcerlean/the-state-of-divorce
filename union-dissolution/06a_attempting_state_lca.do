...
foreach var in unemployment_neg min_above_fed paid_leave cc_pct_income_neg earn_ratio_neg cc_subsidies unemployment cc_pct_income min_below_fed child_pov child_pov_neg min_wage welfare_all welfare_cash_asst welfare_neg welfare_cash_neg ccdf_direct ccdf_per_cap cc_served_percap cc_pct_served educ_spend educ_spend_percap{
	sum `var'
	gen `var'_st = (`var'- `r(mean)') / `r(sd)'
}
...
factor min_above_fed_st paid_leave_st senate_dems_st welfare_all_st educ_spend_percap_st earn_ratio_neg_st, ipf  // ccdf_per_cap_st
predict f1
